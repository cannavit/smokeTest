...
async function executeTestCurl() {
  ...
  for (const key in curlTestList) {
    ...
    outputData   = await checkLogs.detectOnlyStdErrInsideToLogs(dataBeforeToStart,curlTestList, data)
    ...
    for (const key in outputData) {
      ...
      var dataResponse = {
        ...
        apiPath:  data.path,
        curl:     data.testCurl,
        logError: dataLogs.stdErr !== "" ? dataLogs.stdErr : resultOfExec.stdErr,
